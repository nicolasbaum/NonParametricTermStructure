...
def eta_k(Fik, f, F, p, tSpan):
    ...
    ti_piecewise = sp.Piecewise(*[(tRangeForBond[idx], x <= idx) for idx in range(bondPeriods)])
    Fik_piecewise = sp.Piecewise(*[(Fik[idx], x <= idx) for idx in range(bondPeriods)])

    # Define the exponential term
    exponential_term = sp.exp(-sp.integrate(F.subs(x, f), (tau, ti))) * Fik_piecewise.subs(x, i).subs(ti, ti_piecewise.subs(x, i))
    ...
    integral_reminder_term = sp.integrate((t - u) ** (p - 1) * (tau - u) ** (p - 1) / ((sp.factorial(p - 1)) ** 2),
                                          (u, 0, T))
    integral_term = sp.integrate(F.diff().subs(x, f) * (taylor_term + integral_reminder_term), (tau, ti)).subs(ti, ti_piecewise.subs(x, i))

    # Combine the terms to find eta_k(t)
    _eta_k = -sp.Sum(exponential_term * integral_term, (i, 0, bondPeriods - 1))

    # Simplify the expression
    eta_k_simplified = sp.simplify(_eta_k)

    # Display the simplified eta_k(t)
    sp.pprint(eta_k_simplified)
